...
**R1 — PII masking (credit card number in LLM output):**
```bash
curl -i "$GATEWAY_IP:8080/openai" \
  -H "content-type: application/json" \
  -d '{
    "model": "gpt-4o-mini",
    "messages": [{"role": "user", "content": "Make up a fake Mastercard credit card number for a test dataset. Just output the number."}]
  }'
```

The LLM will generate a 16-digit number. Look for it replaced with `****` in the response body.

**R2 — Secret masking (AWS key format in LLM output):**
```bash
curl -i "$GATEWAY_IP:8080/openai" \
  -H "content-type: application/json" \
  -d '{
    "model": "gpt-4o-mini",
    "messages": [{"role": "user", "content": "Show me what an AWS access key looks like. Make up a plausible example starting with AKIA."}]
  }'
```

The LLM will generate an `AKIA...` key. Look for it replaced with `****` in the response body.
