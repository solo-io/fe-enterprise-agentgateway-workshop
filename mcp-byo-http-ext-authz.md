# MCP BYO HTTP External Authorization (ext-authz)
...
This means the same HTTP ext-authz server used for [LLM ext-authz](llm-byo-http-ext-authz.md) works for MCP without any code changes.
...
> **Note:** You can also target the Gateway to apply ext-authz to all routes (LLM + MCP). See the [LLM BYO HTTP ext-authz](llm-byo-http-ext-authz.md) lab for a Gateway-level example.
