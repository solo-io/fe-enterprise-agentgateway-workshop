...
`--reuse-values` is required here. Without it, this upgrade merges the `-f` block below onto the chart's defaults instead of the release's current values, so anything you set outside this lab reverts, including your license key. `--reuse-values` carries those values forward.
