...
Most "give the agent one useful tool" stories require data from several backends. A support agent asking "what's this account's status?" needs a CRM record *and* an open-ticket count *and* maybe a billing balance: three services for one question. **Composable MCP** lets you declare a single MCP tool that fans a call out to distinct backends, runs the necessary steps, and merges the results into one response, so the calling agent never has to know there were multiple hops.
...
Deploy two independent `nginx:alpine` mocks into a dedicated `composable-mcp` namespace. Each serves fixed JSON via nginx's inline `return` directive, with no file mounts or app code, so responses are deterministic:
...
Everything so far has been plumbing: two REST mocks, then an MCP front door for one of them. This step introduces the feature the lab is about: `entMcp.targets[].custom`, a target whose "backend" is not one upstream but a small declarative pipeline of `steps` plus a CEL `output` expression that merges whatever those steps returned into a single tool result.
...
`"Account Globex (tier gold) has 3 open orders."` matches expectations exactly: `output.account.name`/`output.account.tier` came from `accounts-api`'s `/accounts/1` response (`Globex`/`gold`), and `output.orders.open` came from `orders-api`'s `/orders/1` response (`3`). One client call became two upstream calls; the gateway did the fan-out and the merge.
...
The baseline `account-brief` fans out to two HTTP backends. That's aggregation, but the pattern this lab is about is a composite tool that reaches an MCP backend and an HTTP backend in the same call. This step swaps the `account` step from `http` to `mcp`, pointed at the `accounts-mcp` front door built in Step 2, while leaving the `orders` step as plain HTTP against `orders-api`. Everything else (the target name, `inputSchema`, and the `output` CEL) is untouched.
...
`"Account Globex (tier gold) has 3 open orders."` is byte-for-byte the same string Step 3 produced, and the `output` CEL did not change: `output.account.name`/`output.account.tier` resolve straight through even though `account`'s step type flipped from `http` to `mcp`. The composite engine flattens the `mcp` step's result to the same underlying JSON the `accounts-api` HTTP response carried (`{"id":"1","name":"Globex","tier":"gold","region":"us-east","owner":"alice"}`) before handing it to the `output` CEL as `output.account`. It does not leave the step result wrapped in the outer `tools/call` envelope (`content`/`structuredContent`) that Step 2's raw `curl` against `accounts-mcp` showed. A second call confirms this isn't a fluke of `account_id: "1"` specifically:
...
The new `output` is a CEL map literal instead of a CEL string-concatenation expression. It still reads from the same `output.account` and `output.orders` step results Step 5 used (`region` and `tier` were already available, never surfaced before), plus it forwards the raw `orders` array from the `orders` step, and keeps a human-readable `summary` field alongside the structured ones so nothing is lost for a caller that still wants to display text.
...
- **String** result (Steps 3–5): becomes a single `content[0]` item of type `text`.
...
`tools/list` also confirms the caller-facing surface didn't change: the composite tool's `inputSchema` is identical to Step 5's, since this was only an output-shape change on the backend side:
...
> **Takeaway:** a JWT policy and a CEL header together turn `account-brief` from "anyone can ask for anyone's account brief" into "only authenticated callers can ask, and every backend call carries who asked." Scoping the policy to the `HTTPRoute` rather than the shared `Gateway` keeps that enforcement local to `/composable`, and the `jwt` CEL variable makes the caller's validated claims available to *any* step's CEL, not just a gate at the edge; an `apiKey`-authenticated route would expose the same variable as `apiKey` instead. Aggregation and identity use the same mechanism here: CEL expressions with a growing scope (`input`, `output.<step>`, and now `jwt`/`apiKey`) that steps and the final `output` can all read from.

---

## Observe it (optional, requires `002`)

From the caller's side, `account-brief` is one `tools/call`. Behind the gateway it's two upstream requests (the `account` step into `accounts-mcp`/`getAccount`, the `orders` step into `orders-api`), and both observability surfaces from `002` show that fan-out at different granularities.
