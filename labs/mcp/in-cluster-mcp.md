...
|---|---|---|
| Update MCP server image | Must also update Backend if service name changes | Deploy new pods; the gateway auto-discovers them |
...
In this lab we'll use `@modelcontextprotocol/server-everything`, the official MCP reference implementation, which provides tools for testing and exploration:
...
These tools make it easy to verify connectivity, test streaming behavior, and explore the full MCP protocol.
