...
Distributed traces for Claude Desktop requests are surfaced in the Solo UI deployed in the [monitoring tools lab](../installation/002-set-up-ui-and-monitoring-tools.md). The UI ingests OTLP spans from AgentGateway via its built-in OpenTelemetry collector and stores them in ClickHouse.
