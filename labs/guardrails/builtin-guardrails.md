...
Send an innocent request, which should succeed:
```bash
...
Expected: `HTTP 403` with `Request blocked: prompt injection detected.`
...
Expected: `HTTP 403` with `Request blocked: jailbreak attempt detected.`
...
Expected: `HTTP 403` with `Request blocked: system prompt extraction attempt detected.`
...
Expected: `HTTP 422` with `Request blocked: personally identifiable information (PII) detected.`
...
Expected: `HTTP 422` with `Request blocked: credential or secret detected.`
...
Expected: `HTTP 403` with `Request blocked: harmful content request detected.`
...
Expected: `HTTP 403` with `Request blocked: encoding evasion or prompt delimiter injection detected.`
...
Expected: `HTTP 403` with `Request blocked: request contains harmful, hateful, or dangerous advisory content.`
...
The LLM generates something like `testuser@example.com`, but the response returns `HTTP 200` with the address replaced by the recognizer's entity tag:

```json
"content":"<EMAIL_ADDRESS>"
```

Built-in recognizers mask with their entity type (`<EMAIL_ADDRESS>`, `<CREDIT_CARD>`, `<SSN>`, and so on), so the client can still see *what kind* of data was redacted.

**R2, Secret masking (AWS access key in LLM output):**
```bash
curl -i "$GATEWAY_IP:8080/openai" \
  -H "content-type: application/json" \
  -d '{
    "model": "gpt-5.4-nano",
    "messages": [{"role": "user", "content": "What is the AWS documentation example access key ID? Output only the key, nothing else."}]
  }'
```

The LLM generates `AKIAIOSFODNN7EXAMPLE`, which matches the `\bAKIA[0-9A-Z]{16}\b` pattern. The response returns `HTTP 200` with the key replaced:

```json
"content":"<masked>"
```

Custom `matches` patterns mask with the generic `<masked>` placeholder, since a raw regex carries no entity type.
...
Every blocked request is logged with `reason=Guardrail`:

```bash
kubectl logs -n agentgateway-system -l app.kubernetes.io/name=agentgateway-proxy --tail 50 | grep Guardrail
```

```
http.path=/openai http.status=403 protocol=llm error="request rejected by regex guardrail" reason=Guardrail duration=0ms
http.path=/openai http.status=422 protocol=llm error="request rejected by regex guardrail" reason=Guardrail duration=14ms
```

Note the low `duration` values: rejected requests never reach the LLM provider, so you are not billed for blocked traffic. Pattern-only guards evaluate in under a millisecond; the PII guard costs a few milliseconds more because the built-in recognizers scan the full prompt.
...
Open http://localhost:3000 and navigate to **Dashboards > AgentGateway Dashboard**. Rejected requests appear broken out by status code: `403` for policy violations, `422` for PII and credential blocks.
...
Open http://localhost:4000 and click **Tracing** in the left navigation. Blocked requests produce short spans that terminate at the gateway, while requests that pass the guards carry the full LLM attribute set (`gen_ai.request.model`, `gen_ai.usage.input_tokens`, `gen_ai.usage.output_tokens`).
