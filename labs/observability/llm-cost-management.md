...
Each entry's `subject` scopes it to the same dimensions Cost Management attributes spend by: `model`, `provider`, `virtualKey`, `user`, and `group` by default. Entries at different levels compose. The budget below combines a group-wide guardrail that logs overages with a per-user default that blocks them.
...
Expected output: all 8 requests `HTTP 200`. Alice's requests run on `gpt-5.6-luna` and bob's on the pricier `gpt-5.6-terra`, so the dashboard's model pivot has two models to separate. This traffic won't exhaust any of the budgets above.
